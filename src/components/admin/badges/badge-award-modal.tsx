 ...
 * Badge Award Modal
 *
 * Modal for awarding badges to users
 */
...
export function BadgeAwardModal({ badge, onSuccess, onCancel }: BadgeAwardModalProps) {
	...
	const isTemporary = badge.badgeType === 'temporary';
	...
	const handleSubmit = async (e: React.FormEvent) => {
		...
		if (isTemporary && !expiresAt) {
			setError('Для временных бэджиков необходимо указать дату истечения');
			return;
		}
		...
		try {
			...
			setError(err instanceof Error ? err.message : 'Не удалось выдать бэйджик');
		} finally {
	...
	return (
		<div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
			...
			<div className="glass-card bg-[var(--color-secondary)] relative max-w-md w-full p-6 shadow-2xl border border-white/10 animate-in fade-in zoom-in-95 duration-200">
				<h2 className="mb-4 text-2xl font-bold text-white">Выдать бэйджик</h2>
				<p className="text-sm text-white/60 mb-6">
					Выдача бэйджа: <span className="font-medium text-white">{badge.name}</span>
				</p>
				...
				<form onSubmit={handleSubmit} className="space-y-4">
					...
					{/* Expires At (for temporary badges) */}
					{isTemporary && (
						<div>
							<label className="block text-sm font-medium text-white/90 mb-2">
								Дата истечения <span className="text-red-400">*</span>
							</label>
							<Input
								type="datetime-local"
								value={expiresAt}
